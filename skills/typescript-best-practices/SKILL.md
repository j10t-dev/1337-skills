...
Invoke the `principle-type-system-discipline` skill first; this skill grounds that principle in TypeScript syntax.
...
| Boundary validation | Parse where data crosses in, into a named domain type. `Record<string, unknown>` (however spelled) stops at that parse. Trust types inside. Invoke the `principle-boundary-discipline` skill. |
