...
1. Inspect models and roles exposed by the active harness. Trust the harness and delegation tool for availability, resolution, validation, and inheritance. Choose the smallest reliable option; a matching role needs no model override.
   ...
   - Explicitly decide on the appropriate thinking level for subagents. Luna doing non-trivial impl should use max; sol and terra should use high for review and design. Medium for complex impl. 
   - pi only has access to openai, claude to anthropic
