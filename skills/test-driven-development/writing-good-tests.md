...
**Behaviour, not text.** Asserting that a script, skill, or config
contains an exact line proves only that the source is the source. Run
scripts against controlled inputs and assert outputs, side effects, or
exit codes. Documents that instruct agents are tested by the consuming
agent's behaviour (`writing-for-agents`); prose for humans earns no test at
all.
...
is the framework's test, not yours). When upstream behaviour genuinely
surprised you, write one narrow characterisation test naming the
assumption. The same boundary applies inside your code: constructors,
getters, constants, and trivial forwarding earn tests only when they
validate, normalise, default, derive, enforce, or cause side effects —
...
component. Assert the real component's behaviour; if the mock is what you
...
**Your human partner's correction:** "Are we testing the behaviour of a
mock?"
...
"complete" means. Ship the tests the behaviour needs and only those:
...
A mutation nothing catches marks the behaviour as unprotected — or the
