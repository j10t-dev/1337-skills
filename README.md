...
- **Controller finding adjudication and repairs.** Controllers can reject factually false findings at any round with durable evidence, reason and cost if wrong, without a replacement passing verdict. Genuine Critical/Important defects require fixes and re-review; three rounds ends in user escalation, not controller deferral. Original findings and rulings remain visible to independent final review. Scoped implementers report unrelated failures; controllers manage separate repair requirements, checks and review. A bounded blocking repair may share the active change under dual-scope review and a recorded prospective plan-subject correction, without new VCS permissions. Non-blocking repair work must resolve before delivery unless the user changes that requirement. Inline execution retains verification and final review without mandatory per-task subagents.
