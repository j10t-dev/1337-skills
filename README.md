...
- **External docs repository.** Approved designs and plans remain in `$DOCS_ROOT/$projectName/{designs,plans}/`.
...
- **Verification and review.** `skills/shared/verification.md` owns portable baseline, evidence-reuse and failure-handling policy. Establish the baseline during design exploration before approval, or before direct implementation when there is no design phase. Planning carries that evidence and specifies checks; implementation reruns missing or invalidated checks, and completion checks supported claims. Investigate material, repeatable timing changes or timeouts rather than ordinary variation. Reviewers own code review; controllers use their verdicts rather than reviewing again. Fix and re-review until clean, then absorb final fixes into the relevant feature commits. Agree broader repairs with the user.
- **Conditional SDD references.** Root `SKILL.md` defines execution and the durable `.agents/sdd/progress.md` record. Read `review-handling.md` for feedback or blockers and `recovery.md` when resuming.
