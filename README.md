...
- **Two-path brainstorming.** `brainstorming` routes between a direct path for one-session, single-task work that implements without a design or plan, and the existing full design-and-plan path. Prototyping remains a manually invoked standalone rather than a third brainstorming path.
...
- **Matt Pocock workflow skills (third-party additions).** `grill-me`, `grill-with-docs`, `grilling`, `domain-modeling`, and `prototype` come from [Matt Pocock's skills](https://github.com/mattpocock/skills). `prototype` was imported from revision `6654f6b60cd9d5be8b54c6fafe44346dabeb3b76` with its logic and UI branches. It is manually invoked, uses jj and this pack's VCS authority rules, and requires the user to choose the result. Router wording is harness-neutral. `domain-modeling` maintains `CONTEXT.md` terminology only; ADR creation is deliberately excluded because this pack's design workflow owns decision documentation.
